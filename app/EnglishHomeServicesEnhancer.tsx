...
const englishCardId = "iboren-english-window-cleaning-card";
const swedishCardId = "iboren-swedish-window-cleaning-card";
...
export default function EnglishHomeServicesEnhancer() {
  ...
  useEffect(() => {
    ...
    const cardId = isEnglish ? englishCardId : swedishCardId;
    const href = isEnglish ? "/en/window-cleaning" : "/fonsterputs";

    if (document.getElementById(cardId) || grid.querySelector(`a[href="${href}"]`)) return;
    ...
    ` : `
      <div class="mb-20 flex items-start justify-between">
        <div class="grid h-14 w-14 place-items-center rounded-full border border-gold/30 bg-gold/10 text-gold text-2xl">✦</div>
        <span class="rounded-full border border-gold/20 px-3 py-1 text-[10px] font-bold uppercase tracking-[.2em] text-gold/80">skräddarsydd offert</span>
      </div>
      <p class="mb-3 text-[11px] font-bold uppercase tracking-[.28em] text-porcelain/42">04</p>
      <h3 class="display text-4xl font-normal uppercase text-porcelain">Fönsterputs</h3>
      <p class="mt-4 leading-7 text-porcelain/62">För fönster, glasytor och extra städning.</p>
    `;
