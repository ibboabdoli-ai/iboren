...
import { ArrowUpRight, Building2, CalendarCheck2, CheckCircle2, Copy, Home, LocateFixed, Loader2, Mail, Menu, Send, ShieldCheck, Sparkles, Truck, X } from "lucide-react";
...
const services = [
  { icon: Home, title: "Hemstädning", price: "från 399 SEK", body: "För återkommande eller enstaka städning hemma. CleanAI samlar yta, frekvens och särskilda önskemål." },
  { icon: Truck, title: "Flyttstädning", price: "offert efter yta", body: "För flytt, överlämning och checklista. Kunden anger kvm, adress och önskat datum för snabb offert." },
  { icon: Building2, title: "Kontorsstädning", price: "skräddarsydd offert", body: "För företag, lokaler och återkommande service. Flödet samlar yta, tider och åtkomstbehov." }
];
...
export default function HomePage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage(result.message || "Din bokningsförfrågan är registrerad.");
    } catch (error) {
      setStatus("error");
  ...
  return (
    <main className="min-h-screen bg-cream text-ink">
      ...
      <section id="services" className="py-24 md:py-32"><div className="luxe-container"><p className="eyebrow">I / Services</p><h2 className="display mt-4 text-5xl font-bold leading-[.9] text-burgundy md:text-7xl">The cleaning collection.</h2><div className="mt-12 grid gap-5 md:grid-cols-3">{services.map((service) => { const Icon = service.icon; return <article key={service.title} className="relative overflow-hidden rounded-[28px] bg-porcelain p-7 shadow-lg transition hover:-translate-y-1"><div className="mb-20 flex items-start justify-between"><div className="grid h-14 w-14 place-items-center rounded-full bg-burgundy text-porcelain"><Icon size={25} /></div><span className="rounded-full border border-burgundy/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[.2em] text-burgundy/70">{service.price}</span></div><h3 className="display text-4xl font-bold text-burgundy">{service.title}</h3><p className="mt-4 leading-7 text-ink/65">{service.body}</p></article>; })}</div></div></section>
