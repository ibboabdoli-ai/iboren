...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-night text-porcelain">
      ...
      <section id="services" className="bg-night py-24 md:py-32"><div className="luxe-container"><p className="text-[11px] font-bold uppercase tracking-[.38em] text-gold">I / Tjänster</p><h2 className="display mt-4 max-w-4xl text-5xl font-normal uppercase leading-[.9] text-porcelain md:text-7xl">Städtjänster för hem och företag.</h2><div className="mt-12 grid gap-5 md:grid-cols-3">{services.map((service, index) => { const Icon = service.icon; return <Link href={service.href} key={service.title} className="iboren-card-glass iboren-card-glass-hover group relative overflow-hidden rounded-[2rem] p-7"><div className="mb-20 flex items-start justify-between"><div className="iboren-gold-accent grid h-14 w-14 place-items-center rounded-full border border-gold/30 bg-gold/10"><Icon size={25} /></div><span className="iboren-gold-accent rounded-full border border-gold/20 px-3 py-1 text-[10px] font-bold uppercase tracking-[.2em]">{service.price}</span></div><p className="iboren-text-muted-dark mb-3 text-[11px] font-bold uppercase tracking-[.28em]">0{index + 1}</p><h3 className="display text-4xl font-normal uppercase text-[var(--ib-text)]">{service.title}</h3><p className="iboren-text-muted-dark mt-4 leading-7">{service.body}</p></Link>; })}</div></div></section>

      <section id="process" className="iboren-section-dark py-24 md:py-32"><div className="luxe-container"><p className="iboren-gold-accent text-[11px] font-bold uppercase tracking-[.38em]">II / Så fungerar det</p><h2 className="display mt-4 max-w-4xl text-5xl font-normal uppercase leading-[.9] text-[var(--ib-text)] md:text-7xl">Fyra steg. En tydlig förfrågan.</h2><div className="mt-12 grid gap-4 md:grid-cols-4">{["Välj tjänst", "Fyll i plats", "Se sammanfattning", "Skicka förfrågan"].map((item, i) => <article key={item} className="iboren-card-glass iboren-card-glass-hover rounded-[2rem] p-6"><div className="mb-10 flex items-center justify-between"><span className="iboren-gold-accent display text-4xl">0{i + 1}</span><CheckCircle2 className="iboren-gold-accent" /></div><h3 className="display text-3xl font-normal uppercase text-[var(--ib-text)]">{item}</h3><p className="iboren-text-muted-dark mt-4 text-sm leading-7">Ett enkelt steg som gör bokningsunderlaget tydligare och lättare att följa upp.</p></article>)}</div></div></section>
