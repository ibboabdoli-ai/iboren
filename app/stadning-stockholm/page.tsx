    ...
    <main className="min-h-screen bg-cream text-ink">
      ...
      <section className="relative overflow-hidden py-20 md:py-28">
        ...
        <div className="luxe-container relative grid gap-10 md:grid-cols-[1fr_.9fr] md:items-center">
          <div>
            ...
            <h1 className="display mt-4 text-5xl font-bold leading-[.9] text-burgundy md:text-8xl">Städning i Stockholm</h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-ink/70 md:text-xl">Iboren hjälper kunder i Stockholm att skapa tydliga bokningsförfrågningar för hemstädning, flyttstädning, kontorsstädning och fönsterputs.</p>
          ...
          <aside className="rounded-[2.5rem] bg-ink p-8 text-porcelain shadow-luxe">
            <div className="mb-12 grid h-16 w-16 place-items-center rounded-full bg-gold text-ink"><Sparkles size={30} /></div>
            <h2 className="display text-4xl font-bold text-gold">Strukturerad bokning för en större stad.</h2>
            <div className="mt-7 grid gap-4">
              {["Tjänst och område", "Adress och praktiska detaljer", "Storlek och omfattning", "Datum, tidsfönster och kontakt"].map((item) => <p key={item} className="flex gap-3 text-porcelain/72"><CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-gold" /> {item}</p>)}
            </div>
          </aside>
        </div>
      </section>
      ...
      <section className="bg-porcelain py-16">
        <div className="luxe-container">
          <p className="eyebrow">Tjänster i Stockholm</p>
          <h2 className="display mt-3 text-4xl font-bold text-burgundy md:text-6xl">Städning för hem, flytt och arbetsplats.</h2>
      ...
      <section className="bg-cream py-16">
        <div className="luxe-container max-w-4xl">
          ...
          <h2 className="display mt-3 text-4xl font-bold text-burgundy md:text-6xl">Vanliga frågor om städning i Stockholm.</h2>
          ...
          <div className="mt-10 rounded-[2rem] bg-burgundy p-7 text-porcelain"><h2 className="display text-4xl font-bold">Skicka en tydlig förfrågan.</h2><p className="mt-3 text-porcelain/70">Fyll i tjänst, plats, datum och önskemål så återkommer Iboren.</p><Link href="/#booking" className="mt-6 inline-flex rounded-full bg-gold px-5 py-3 text-sm font-black uppercase tracking-[.12em] text-ink">Starta bokning</Link></div>
        </div>
      </section>
    </main>
  );
