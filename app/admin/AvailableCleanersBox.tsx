...
type AssignmentResponse = {
  ...
  previousCleanerEmail?: CleanerEmailResult;
};
...
function emailStatusMessage(employee: Suggestion["employee"], cleanerEmail?: CleanerEmailResult, previousCleanerEmail?: CleanerEmailResult) {
  const assigned = cleanerEmail?.sent ? `Email sent to ${employee.email}.` : cleanerEmail?.skipped ? `Cleaner email was not sent: ${cleanerEmail.reason || "skipped"}.` : cleanerEmail ? `Cleaner email failed: ${cleanerEmail.reason || "unknown error"}.` : "Email status unknown.";
  const previous = previousCleanerEmail?.sent ? " Previous cleaner was notified." : previousCleanerEmail && !previousCleanerEmail.skipped ? ` Previous cleaner notification failed: ${previousCleanerEmail.reason || "unknown error"}.` : "";
  return `Assigned to ${employee.name}. ${assigned}${previous}`;
}
...
export default function AvailableCleanersBox({ bookingId, getToken }: { bookingId: string; getToken: () => Promise<string | null> }) {
  ...
  async function assignCleaner(employee: Suggestion["employee"]) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/bookings/${bookingId}/assignment`, { method: "POST", headers, body: JSON.stringify({ employee_id: employee.id }) });
      const result = await response.json().catch(() => null) as AssignmentResponse | null;
      ...
      setMessage(emailStatusMessage(assigned, result.cleanerEmail, result.previousCleanerEmail));
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not assign cleaner."); }
    setAssigningId(null);
  }
  ...
  return (
    <div className="mt-4 rounded-2xl bg-porcelain p-4 text-sm shadow-sm ring-1 ring-burgundy/10">
      ...
      {assignedEmployee && <div className={`mt-3 rounded-xl p-3 text-xs font-bold ${assignmentStatusClass(assignmentStatus)}`}><span className="inline-flex flex-wrap items-center gap-2"><UserCheck className="h-4 w-4" /> Assigned cleaner: {assignedEmployee.name} · {assignedEmployee.email} · status: {assignmentStatusLabel(assignmentStatus)}</span></div>}
      {assignedEmployee && assignmentStatus !== "completed" && <p className="mt-2 text-xs font-bold text-ink/45">To reassign, choose another cleaner below. The previous cleaner will be notified.</p>}
      {assignmentStatus === "completed" && <p className="mt-2 text-xs font-bold text-ink/45">Completed assignments cannot be reassigned here.</p>}
      ...
      {loading && !loaded ? <div className="mt-4 grid min-h-16 place-items-center text-burgundy"><Loader2 className="h-5 w-5 animate-spin" /></div> : suggestions.length === 0 ? <p className="mt-4 rounded-xl bg-cream p-3 text-xs font-bold text-ink/55">No available cleaner found for this date/time yet.</p> : (
        <div className="mt-4 grid gap-3">
          {suggestions.map((suggestion) => {
            ...
            const isLocked = assignmentStatus === "completed";
            return <article key={suggestion.employee.id} className={`rounded-xl p-3 ${isAssigned ? "bg-green-50 ring-1 ring-green-200" : "bg-cream"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black text-burgundy">{suggestion.employee.name}</p><p className="text-xs text-ink/55">{suggestion.employee.email}{suggestion.employee.phone ? ` · ${suggestion.employee.phone}` : ""}</p></div><div className="flex flex-col items-start gap-2 sm:items-end"><p className="text-xs font-bold text-ink/55">Car: {suggestion.employee.has_car ? "Yes" : "No"} · Max {suggestion.employee.max_hours_per_day}h/day</p><button type="button" disabled={isAssigned || Boolean(assigningId) || isLocked} onClick={() => assignCleaner(suggestion.employee)} className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 text-xs font-black uppercase tracking-[.12em] disabled:opacity-60 ${isAssigned ? "bg-green-100 text-green-800" : "bg-burgundy text-porcelain"}`}>{isAssigning ? <Loader2 className="h-4 w-4 animate-spin" /> : isAssigned ? <CheckCircle2 className="h-4 w-4" /> : <UserCheck className="h-4 w-4" />}{isAssigned ? "Assigned" : isAssigning ? "Assigning" : assignedEmployeeId ? "Reassign" : "Assign"}</button></div></div><div className="mt-2 flex flex-wrap gap-2">{suggestion.slots.map((slot) => <span key={slot.id} className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800 ring-1 ring-green-200">{formatTime(slot.start_time)}–{formatTime(slot.end_time)}</span>)}</div></article>;
          })}
        </div>
      )}
    </div>
