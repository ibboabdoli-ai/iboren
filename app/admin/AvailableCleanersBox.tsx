...
import { CheckCircle2, Loader2, RefreshCw, Save, Send, UserCheck, UsersRound, XCircle } from "lucide-react";
...
type CrewResponse = { ok?: boolean; message?: string; cleaners_needed?: number; needsMigration?: boolean };
...
export default function AvailableCleanersBox({ bookingId, getToken }: { bookingId: string; getToken: () => Promise<string | null> }) {
  ...
  async function loadCrew() { const headers = await authHeaders(false); const response = await fetch(`/api/admin/bookings/${bookingId}/crew`, { headers }); const result = await response.json().catch(() => null) as CrewResponse | null; if (!response.ok || !result?.ok) throw new Error(result?.message || "Could not load cleaners needed."); setCleanersNeeded(result.cleaners_needed || 1); setCrewMigrationNeeded(Boolean(result.needsMigration)); if (result.needsMigration && result.message) setMessage(result.message); }
  ...
  async function saveCrewNeeded() {
    setSavingCrew(true); setMessage("");
    try {
      const headers = await authHeaders(true);
      const response = await fetch(`/api/admin/bookings/${bookingId}/crew`, { method: "PATCH", headers, body: JSON.stringify({ cleaners_needed: cleanersNeeded }) });
      const result = await response.json().catch(() => null) as CrewResponse | null;
      if (!response.ok || !result?.ok) throw new Error(result?.message || "Could not save cleaners needed.");
      setCleanersNeeded(result.cleaners_needed || cleanersNeeded);
      setCrewMigrationNeeded(false);
      setMessage(`Cleaners needed saved: ${result.cleaners_needed || cleanersNeeded}.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save cleaners needed."); }
    setSavingCrew(false);
  }
  ...
  const confirmedCount = offers.filter((offer) => ["confirmed", "completed"].includes(offer.assignment.status)).length;
  ...
  return (
    <div className="mt-4 rounded-2xl bg-porcelain p-4 text-sm shadow-sm ring-1 ring-burgundy/10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="inline-flex items-center gap-2 font-black text-burgundy"><UsersRound className="h-4 w-4" /> Available cleaners</p><p className="mt-1 text-xs font-bold text-ink/45">{weekday ? weekdays[weekday] : "Booking date"}</p></div><button type="button" onClick={loadSuggestions} className="inline-flex items-center justify-center gap-2 rounded-full bg-cream px-4 py-2 text-xs font-black uppercase tracking-[.12em] text-burgundy">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}Refresh</button></div>

      <div className="mt-3 rounded-2xl bg-cream p-3 ring-1 ring-burgundy/10"><div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-black uppercase tracking-[.14em] text-ink/45">Crew planning</p><div className="mt-2 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[.12em]"><span className="rounded-full bg-porcelain px-3 py-1 text-ink/70">Confirmed {confirmedCount}/{cleanersNeeded}</span><span className="rounded-full bg-green-100 px-3 py-1 text-green-800 ring-1 ring-green-200">Available {availableCount}</span><span className="rounded-full bg-gold px-3 py-1 text-ink">Offers {sentCount}</span></div></div><div className="flex items-center gap-2"><label className="text-xs font-black uppercase tracking-[.12em] text-ink/45">Cleaners needed</label><input type="number" min={1} max={20} value={cleanersNeeded} onChange={(event) => setCleanersNeeded(Number(event.target.value))} className="w-20 rounded-xl border border-burgundy/10 bg-porcelain px-3 py-2 text-sm font-bold text-ink" /><button type="button" disabled={savingCrew || crewMigrationNeeded} onClick={saveCrewNeeded} className="inline-flex items-center gap-2 rounded-full bg-burgundy px-4 py-2 text-xs font-black uppercase tracking-[.12em] text-porcelain disabled:opacity-50">{savingCrew ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save</button></div></div>{crewMigrationNeeded && <p className="mt-2 text-xs font-bold text-red-800">Run the Step 18F Supabase SQL before saving cleaners needed.</p>}</div>
      ...
      {loading && !loaded ? <div className="mt-4 grid min-h-16 place-items-center text-burgundy"><Loader2 className="h-5 w-5 animate-spin" /></div> : suggestions.length === 0 ? <p className="mt-4 rounded-xl bg-cream p-3 text-xs font-bold text-ink/55">No available cleaner found for this date/time yet.</p> : <div className="mt-4 grid gap-3">{suggestions.map((suggestion) => { const offer = offers.find((item) => item.assignment.employee_id === suggestion.employee.id); const isSending = sendingId === suggestion.employee.id; const locked = Boolean(offer && ["completed", "confirmed", "not_selected"].includes(offer.assignment.status)); return <article key={suggestion.employee.id} className={`rounded-xl p-3 ${offer ? "bg-green-50 ring-1 ring-green-200" : "bg-cream"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black text-burgundy">{suggestion.employee.name}</p><p className="text-xs text-ink/55">{suggestion.employee.email}{suggestion.employee.phone ? ` · ${suggestion.employee.phone}` : ""}</p></div><div className="flex flex-col items-start gap-2 sm:items-end"><p className="text-xs font-bold text-ink/55">Car: {suggestion.employee.has_car ? "Yes" : "No"} · Max {suggestion.employee.max_hours_per_day}h/day</p><button type="button" disabled={Boolean(sendingId) || locked} onClick={() => sendOffer(suggestion.employee)} className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 text-xs font-black uppercase tracking-[.12em] disabled:opacity-60 ${offer ? "bg-green-100 text-green-800" : "bg-burgundy text-porcelain"}`}>{isSending ? <Loader2 className="h-4 w-4 animate-spin" /> : offer ? <CheckCircle2 className="h-4 w-4" /> : <Send className="h-4 w-4" />}{isSending ? "Sending" : offer ? statusLabel(offer.assignment.status) : "Send offer"}</button></div></div><div className="mt-2 flex flex-wrap gap-2">{suggestion.slots.map((slot) => <span key={slot.id} className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800 ring-1 ring-green-200">{formatTime(slot.start_time)}–{formatTime(slot.end_time)}</span>)}</div></article>; })}</div>}
    </div>
  );
}
