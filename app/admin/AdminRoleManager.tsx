...
import { Loader2, RefreshCw, Save, ShieldCheck, UsersRound } from "lucide-react";
    ...
    <section className="mt-6 rounded-[2rem] bg-porcelain p-5 shadow-soft md:p-7">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl leading-7 text-ink/65">Add cleaners, supervisors and admins without using Supabase SQL. Customers do not need a manual role.</p>
        </div>
      ...
      <form onSubmit={saveRole} className="mt-6 grid gap-4 rounded-[1.6rem] bg-cream p-5 lg:grid-cols-2 xl:grid-cols-4">
        <label className="block xl:col-span-2"><span className="mb-2 block text-sm font-bold">Email</span><input value={email} onChange={(event) => setEmail(event.target.value)} type="email" required placeholder="cleaner@example.com" className="w-full rounded-2xl border border-burgundy/10 bg-porcelain px-4 py-3" /></label>
        ...
        <button disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-full bg-burgundy px-5 py-3 text-sm font-black uppercase tracking-[.12em] text-porcelain disabled:opacity-60"><Save className="h-4 w-4" /> {saving ? "Saving..." : "Save role"}</button>
      </form>
      ...
      <div className="mt-6 grid gap-3">
        {loading ? <div className="grid min-h-24 place-items-center text-burgundy"><Loader2 className="h-6 w-6 animate-spin" /></div> : rolesList.length === 0 ? <p className="rounded-2xl bg-cream p-4 text-sm text-ink/65">No roles found.</p> : rolesList.map((item) => {
          ...
          return (
            <article key={item.id} className="grid gap-3 rounded-[1.5rem] border border-burgundy/10 bg-cream p-4 text-sm md:grid-cols-[1fr_auto] md:items-center">
              <div>
              ...
              <button type="button" onClick={() => { setEmail(item.email); setRole(item.role); setActive(item.active); setName(employee?.name || ""); setPhone(employee?.phone || ""); setHasCar(Boolean(employee?.has_car)); setMaxHours(String(employee?.max_hours_per_day || 8)); }} className="inline-flex items-center justify-center gap-2 rounded-full bg-porcelain px-4 py-2 font-bold text-burgundy"><ShieldCheck className="h-4 w-4" /> Edit</button>
            </article>
          );
        })}
      </div>
