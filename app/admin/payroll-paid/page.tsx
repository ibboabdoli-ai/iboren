...
function toCsv(start: string, end: string, summaries: Summary[]) {
  const header = ["period_start", "period_end", "employee_name", "employee_email", "approved_entries", "worked_hours", "break_minutes", "travel_minutes", "mileage_km"];
  const rows = summaries.map((item) => [start, end, item.employee_name, item.employee_email, item.approved_entries, hours(item.worked_minutes), item.break_minutes, item.travel_minutes, item.mileage_km]);
...
export default function PayrollPaidPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setMessage("Paid payroll loaded.");
  ...
  return <main className="min-h-screen bg-cream py-12 text-ink"><section className="luxe-container"><Link href="/admin/payroll-basis" className="text-sm font-bold text-burgundy">← Back to open payroll</Link><div className="mt-6 rounded-[2rem] bg-burgundy p-7 text-porcelain"><p className="text-xs font-bold uppercase tracking-[.25em] text-gold">Iboren Admin</p><h1 className="display mt-3 text-5xl font-bold">Paid payroll archive</h1><p className="mt-4 text-porcelain/70">View entries already moved to paid and copy accountant CSV.</p></div><div className="mt-6 rounded-[2rem] bg-porcelain p-5 shadow-soft"><div className="grid gap-3 md:grid-cols-3"><label><span className="block text-xs font-black uppercase tracking-[.12em] text-ink/45">Start</span><input type="date" value={start} onChange={(e) => setStart(e.target.value)} className="mt-1 w-full rounded-xl bg-cream px-3 py-2 font-bold" /></label><label><span className="block text-xs font-black uppercase tracking-[.12em] text-ink/45">End</span><input type="date" value={end} onChange={(e) => setEnd(e.target.value)} className="mt-1 w-full rounded-xl bg-cream px-3 py-2 font-bold" /></label><button onClick={load} className="rounded-full bg-burgundy px-5 py-3 font-bold text-porcelain md:self-end">{loading ? "Loading..." : "Load paid"}</button></div>{message && <p className="mt-4 rounded-xl bg-burgundy/10 p-3 font-bold text-burgundy">{message}</p>}<div className="mt-5 grid gap-3 md:grid-cols-5"><p className="rounded-xl bg-cream p-3 font-bold">Entries<br />{total.entries}</p><p className="rounded-xl bg-cream p-3 font-bold">Work<br />{hours(total.worked)}h</p><p className="rounded-xl bg-cream p-3 font-bold">Break<br />{total.breakM}m</p><p className="rounded-xl bg-cream p-3 font-bold">Travel<br />{hours(total.travel)}h</p><p className="rounded-xl bg-cream p-3 font-bold">Mileage<br />{total.mileage}km</p></div><div className="mt-5 grid gap-3">{summaries.map((item) => <article key={item.employee_id} className="rounded-xl bg-cream p-4"><h2 className="display text-2xl font-bold text-burgundy">{item.employee_name}</h2><p className="text-sm font-bold text-ink/55">{item.employee_email}</p><p className="mt-3 text-sm font-bold">Entries: {item.approved_entries} · Work: {hours(item.worked_minutes)}h · Break: {item.break_minutes}m · Travel: {item.travel_minutes}m · Mileage: {item.mileage_km}km</p></article>)}{!loading && summaries.length === 0 && <p className="rounded-xl bg-cream p-4 font-bold text-ink/55">No paid entries in this period.</p>}</div>{csvText && <div className="mt-5 rounded-2xl bg-cream p-3"><button type="button" onClick={copyCsv} className="mb-3 rounded-full bg-porcelain px-4 py-2 text-xs font-black uppercase tracking-[.12em] text-burgundy ring-1 ring-burgundy/10">Copy CSV</button><textarea readOnly value={csvText} className="h-64 w-full rounded-xl bg-porcelain p-3 font-mono text-xs leading-6 text-ink/80 outline-none" /></div>}</div></section></main>;
}
