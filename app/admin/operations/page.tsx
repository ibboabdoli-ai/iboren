...
export default function AdminOperationsPage() {
  ...
  const stats = useMemo(() => getWorkflowStats(bookings), [bookings]);
    ...
    <main className="min-h-screen bg-cream py-10 text-ink md:py-14">
      <section className="luxe-container">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={loadBookings} className="inline-flex rounded-full bg-burgundy px-5 py-3 text-sm font-bold text-porcelain shadow-soft disabled:opacity-60" disabled={bookingsLoading}>{bookingsLoading ? "Uppdaterar..." : "Uppdatera"}</button>
        </div>
        ...
        <div className="mt-6 rounded-[2.2rem] bg-burgundy p-7 text-porcelain shadow-luxe md:p-9">
          ...
          <p className="mt-4 max-w-2xl leading-8 text-porcelain/72">Operativ vy för vad som behöver beslut nu. Den ändrar inte bokningar, statusar, formulär eller databasen.</p>
        </div>
