...
function riskText(risk: RiskLevel) {
  if (risk === "Red") return "Manual quote needed";
  if (risk === "Yellow") return "Needs review";
...
function riskBadgeClass(risk: RiskLevel) {
  ...
  if (risk === "Red") return `${base} border-red-300/45 bg-red-400/20 text-red-100`;
  if (risk === "Yellow") return `${base} border-amber-300/45 bg-amber-400/20 text-amber-100`;
  return `${base} border-emerald-300/45 bg-emerald-400/20 text-emerald-100`;
}

function estimatePrice(input: EstimateInput): Estimate {
  const selectedAddOns = input.service === "Office cleaning" ? [] : input.service === "Window cleaning" ? input.selectedAddOns.filter((item) => item !== "Window cleaning") : input.selectedAddOns;
  ...
  if (input.service === "Home cleaning") {
    ...
    return { title: "Estimated price for home cleaning", beforeRut, afterRut: beforeRut * rutFactor, hours, addOnsBeforeRut, riskLevel: riskLevel(input), factors, note: "Estimate based on size, bathrooms, rooms, condition, access, frequency and add-ons. Final price is confirmed before the request becomes binding." };
  }
...
export default function EnglishPriceCalculator() {
  ...
  const showOfficeFields = service === "Office cleaning";
  const showMoveFields = service === "Move-out cleaning";
  const showWindowFields = service === "Window cleaning" || selectedAddOns.includes("Window cleaning");
  const showAddOns = !showOfficeFields;
  const visibleAddOns = service === "Window cleaning" ? addOns.filter((item) => item !== "Window cleaning") : addOns;
  ...
  return (
    <section className="rounded-[2rem] border border-burgundy/10 bg-porcelain p-6 shadow-luxe md:p-8" id="price-calculator">
      <div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.28em] text-burgundy">Calculator</p><h2 className="display mt-2 text-4xl font-bold text-burgundy">Get estimate</h2><p className="mt-2 text-sm leading-6 text-ink/65">Detailed price estimate. Final price is always confirmed before booking.</p></div><div className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-burgundy text-porcelain"><Calculator /></div></div>
      <div className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3"><NumberField label="Size sqm" value={sqm} onChange={setSqm} />{!showOfficeFields && <NumberField label="Rooms" value={rooms} onChange={setRooms} />}{!showOfficeFields && <NumberField label="Bathrooms" value={bathrooms} onChange={setBathrooms} />}</div>
        <div className="grid gap-4 sm:grid-cols-2">{!showOfficeFields && <SelectField label="Frequency" value={frequency} options={frequencies} onChange={(value) => setFrequency(value as Frequency)} />}<label className="block"><span className="mb-2 block text-sm font-bold">Postal code</span><input value={postalCode} onChange={(event) => setPostalCode(event.target.value.slice(0, 12))} className="w-full rounded-2xl border border-burgundy/10 bg-cream px-4 py-4" placeholder="151 46" /></label></div>
        <div className="grid gap-4 sm:grid-cols-2"><SelectField label="Condition" value={condition} options={conditions} onChange={(value) => setCondition(value as Condition)} />{showMoveFields && <SelectField label="Move-out status" value={furnished} options={furnishedOptions} onChange={(value) => setFurnished(value as Furnished)} />}</div>
        {!showOfficeFields && <div className="grid gap-4 sm:grid-cols-3"><SelectField label="Pets" value={pets} options={yesNoOptions} onChange={(value) => setPets(value as YesNo)} /><NumberField label="Floor" value={floor} onChange={setFloor} /><SelectField label="Elevator" value={elevator} options={yesNoOptions} onChange={(value) => setElevator(value as YesNo)} /></div>}
