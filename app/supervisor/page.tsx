...
export default function SupervisorPage() {
  ...
  async function loadJobs() {
    ...
    try {
      ...
      const response = await fetch("/api/cleaner/jobs", { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <main className="min-h-screen bg-cream py-12 text-ink md:py-16">
      <section className="luxe-container">
        <Link href="/profile" className="mb-8 inline-flex items-center gap-2 text-sm font-bold text-burgundy"><ArrowLeft size={17} /> Back to profile</Link>
