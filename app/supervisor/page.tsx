    ...
    <main className="min-h-screen bg-cream py-12 text-ink md:py-16">
      <section className="luxe-container">
        ...
        <div className="rounded-[2.5rem] bg-burgundy p-7 text-porcelain shadow-luxe md:p-9">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-5 max-w-2xl leading-8 text-porcelain/70">Read-only overview for today and the next 7 days.</p>
            </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <Stat title="Loaded jobs" value={jobs.length} />
        </div>

        <JobList title="Today’s jobs" jobs={todaysJobs} loading={jobsLoading} />
        <JobList title="Next 7 days" jobs={next7Jobs} loading={jobsLoading} />
      </section>
    </main>
  );
}
...
function JobList({ title, jobs, loading }: { title: string; jobs: Job[]; loading: boolean }) {
  return (
    <section className="mt-6 rounded-[2rem] bg-porcelain p-5 shadow-soft md:p-7">
      <div className="mb-5 flex items-center gap-3"><CalendarDays className="h-5 w-5 text-burgundy" /><h2 className="display text-4xl font-bold text-burgundy">{title}</h2></div>
      {loading ? <Loader2 className="h-7 w-7 animate-spin text-burgundy" /> : jobs.length === 0 ? <p className="rounded-2xl bg-cream p-5 text-ink/65">No jobs found.</p> : <div className="grid gap-4">{jobs.map((job) => <JobCard key={job.id} job={job} />)}</div>}
    </section>
  );
}
    ...
    <article className="rounded-[1.5rem] border border-burgundy/10 bg-cream p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[.18em] text-ink/45">{job.preferred_date || "No date"} · {job.time_window || "Flexible"}</p>
          <h3 className="display mt-2 break-words text-3xl font-bold text-burgundy">{job.service}</h3>
      ...
      <div className="mt-4 grid gap-2 rounded-2xl bg-porcelain/70 p-4 text-sm text-ink/65 md:grid-cols-3">
        ...
        <p><strong>Size:</strong> {job.size_sqm ? `${job.size_sqm} kvm` : "—"}</p>
      </div>
    </article>
  );
}
