...
import { ArrowUpRight, Copy, Home, LocateFixed, Loader2, Mail, Menu, Send, ShieldCheck, UserRound, X } from "lucide-react";
...
const displayOptions = new Map<string, string>([
...
function display(value: string) {
  return displayOptions.get(value) || value;
}
...
function buildBookingNotes(draft: BookingDraft) {
  const details = [
    "--- Property & details ---",
    `Property type: ${display(draft.propertyType) || "Not filled in"}`,
    `Rooms: ${draft.rooms || "Not filled in"}`,
    `Bathrooms: ${draft.bathrooms || "Not filled in"}`,
    `Pets: ${display(draft.pets) || "Not filled in"}`,
    `Floor: ${draft.floor || "Not filled in"}`,
    `Elevator: ${display(draft.elevator) || "Not filled in"}`,
    `Parking: ${display(draft.parking) || "Not filled in"}`,
    `Extra services: ${draft.extras.length ? draft.extras.map(display).join(", ") : "None selected"}`
  ];

  if (draft.notes.trim()) details.push("", "--- Customer notes ---", draft.notes.trim());
  return details.join("\n");
}
...
export default function EnglishHomePage() {
  ...
  function toggleExtra(extra: string) {
    setDraft((current) => ({
      ...current,
      extras: current.extras.includes(extra) ? current.extras.filter((item) => item !== extra) : [...current.extras, extra]
    }));
  }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-night text-porcelain">
      ...
      <section id="booking" className="bg-ink py-24 text-porcelain md:py-32">
        <div className="luxe-container grid gap-10 lg:grid-cols-[.82fr_1.18fr]">
          <div>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-porcelain/70">This form is the English copy of the Swedish booking form. Fill in service, location, size, rooms, date, contact details and special requests.</p>
          ...
          <div className="grid gap-5 xl:grid-cols-[1fr_.88fr]">
            <form onSubmit={submit} className="rounded-[2rem] border border-porcelain/10 bg-porcelain/8 p-5 shadow-2xl backdrop-blur-xl md:p-7">
              ...
              <div className="grid gap-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2"><Field label="Preferred date" type="date" value={draft.date} onChange={(v) => setField("date", v)} /><Select label="Time window" value={draft.timeWindow} options={timeOptions} onChange={(v) => setField("timeWindow", v)} /></div>
                <Select label="Frequency" value={draft.frequency} options={frequencyOptions} onChange={(v) => setField("frequency", v)} />
                <div className="grid gap-4 sm:grid-cols-2"><Field label="Name" value={draft.name} onChange={(v) => setField("name", v)} placeholder="Full name" /><Field label="Email" value={draft.email} onChange={(v) => setField("email", v)} placeholder="name@email.se" type="email" /></div>
                <Field label="Phone" value={draft.phone} onChange={(v) => setField("phone", v)} placeholder="+46 ..." type="tel" />
                <textarea value={draft.notes} onChange={(e) => setField("notes", e.target.value)} className="min-h-28 w-full rounded-2xl border border-porcelain/10 bg-porcelain px-4 py-4 text-ink placeholder:text-ink/45 outline-none" placeholder="Special requests..." />
      ...
      <footer className="border-t border-gold/10 bg-night py-10"><div className="luxe-container grid gap-8 md:grid-cols-[1.1fr_1fr_1fr]"><div><p className="display text-4xl font-normal uppercase text-gold">Iboren</p><p className="mt-2 max-w-sm text-sm leading-7 text-porcelain/65">Cleaning in Södertälje and Stockholm with clear pricing, RUT information and simple booking requests.</p></div><div><p className="mb-3 text-xs font-black uppercase tracking-[.22em] text-gold">Iboren</p><div className="grid gap-2 text-sm font-semibold text-porcelain/70"><Link href="/en/prices" className="hover:text-gold">Prices</Link><Link href="/en/jobs" className="hover:text-gold">Work with us</Link><Link href="/en/about" className="hover:text-gold">About us</Link><Link href="/en/privacy" className="hover:text-gold">Privacy</Link><Link href="/en/terms" className="hover:text-gold">Terms</Link><a href="mailto:hej@iboren.se" className="hover:text-gold">hej@iboren.se</a></div></div></div></footer>
    </main>
  );
}
