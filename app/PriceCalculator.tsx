...
export default function PriceCalculator({ locale = "sv" }: { locale?: BookingPricingLocale }) {
  ...
  return (
    <section className="rounded-[2rem] border border-gold/20 bg-[#151815] p-6 text-porcelain shadow-[0_28px_90px_rgba(0,0,0,.28)] md:p-8" id={t.id}>
      <div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.28em] text-gold">{t.eyebrow}</p><h2 className="display mt-2 text-4xl font-bold text-porcelain">{t.heading}</h2><p className="mt-2 text-sm leading-6 text-porcelain/65">{t.intro}</p></div><div className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-gold text-night"><Calculator /></div></div>
      <div className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">{!showOfficeFields && <OptionSelect label={t.frequency} value={frequency} options={frequencyOptions} locale={locale} onChange={(value) => setFrequency(value as Frequency)} />}<label className="block"><span className="mb-2 block text-sm font-bold">{t.postalCode}</span><input value={postalCode} onChange={(event) => setPostalCode(event.target.value.slice(0, 12))} className="w-full rounded-2xl border border-porcelain/15 bg-night px-4 py-4 text-porcelain outline-none transition focus:border-gold" placeholder="151 46" /></label></div>
        ...
        {showAddOns && <div><p className="mb-2 text-sm font-bold">{t.addOns}</p><div className="grid gap-2 sm:grid-cols-2">{visibleAddOns.map((item) => <button type="button" key={item.value} onClick={() => toggleAddOn(item.value)} className={`rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${selectedAddOns.includes(item.value) ? "border-gold bg-gold text-night" : "border-porcelain/15 bg-porcelain/[.06] text-porcelain"}`}>{optionLabel(item, locale)}</button>)}</div></div>}
        {rutEligible ? <label className="flex items-start gap-3 rounded-2xl border border-gold/15 bg-gold/[.08] p-4 text-sm font-bold"><input type="checkbox" checked={rutRequested} onChange={(event) => setRutRequested(event.target.checked)} className="mt-1 h-5 w-5" /><span>{t.showRut}<br /><span className="font-normal text-porcelain/65">{t.rutHelp}</span></span></label> : <p className="rounded-2xl border border-porcelain/12 bg-porcelain/[.06] p-4 text-sm font-bold text-porcelain/72">{t.noRut}</p>}
      </div>
      ...
      <div className="mt-5 grid gap-3 text-sm text-porcelain/72 md:grid-cols-3">{t.bullets.map((item) => <p key={item} className="flex gap-2 rounded-2xl border border-porcelain/12 bg-porcelain/[.06] p-4"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-gold" /> {item}</p>)}</div>
    </section>
  );
}

function NumberField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="block"><span className="mb-2 block text-sm font-bold">{label}</span><input inputMode="numeric" value={value} onChange={(event) => onChange(event.target.value.replace(/[^0-9]/g, ""))} className="w-full rounded-2xl border border-porcelain/15 bg-night px-4 py-4 text-porcelain outline-none transition focus:border-gold" /></label>;
}
...
function OptionSelect<T extends string>({ label, value, options, locale, onChange }: { label: string; value: T; options: BookingPricingOption<T>[]; locale: BookingPricingLocale; onChange: (value: T) => void }) {
  return <label className="block"><span className="mb-2 block text-sm font-bold">{label}</span><select value={value} onChange={(event) => onChange(event.target.value as T)} className="w-full rounded-2xl border border-porcelain/15 bg-night px-4 py-4 text-porcelain outline-none transition focus:border-gold">{options.map((item) => <option key={item.value} value={item.value}>{optionLabel(item, locale)}</option>)}</select></label>;
}
...
function OptionButtons<T extends string>({ label, options, value, locale, onChange }: { label: string; options: BookingPricingOption<T>[]; value: T; locale: BookingPricingLocale; onChange: (value: T) => void }) {
  return <div><p className="mb-2 text-sm font-bold">{label}</p><div className="grid grid-cols-2 gap-2">{options.map((item) => <button type="button" key={item.value} onClick={() => onChange(item.value)} className={`rounded-2xl border px-4 py-3 text-sm font-bold transition ${value === item.value ? "border-gold bg-gold text-night" : "border-porcelain/15 bg-porcelain/[.06] text-porcelain"}`}>{optionLabel(item, locale)}</button>)}</div></div>;
}
