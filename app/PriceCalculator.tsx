...
function estimatePrice(service: Service, sqm: number, frequency: Frequency, bathrooms: number, selectedAddOns: AddOn[], useRut: boolean): Estimate {
  ...
  if (service === "Hemstädning") {
    ...
    return { title: "Uppskattat pris för hemstädning", beforeRut, afterRut: beforeRut * rutFactor, hours, note: "Konkurrenskraftig prisindikation: återkommande hemstädning räknas från cirka 260 kr/tim efter RUT innan frekvensrabatt. Engångsstädning ligger högre eftersom start och genomgång tar mer tid." };
  }
  ...
  if (service === "Flyttstädning") {
    ...
    return { title: "Uppskattat pris för flyttstädning", beforeRut, afterRut: beforeRut * rutFactor, note: "Flyttstädning räknas med fast kvm-modell från cirka 21 kr/kvm efter RUT. Slutpris påverkas av bostadens skick, tillval, fönster, balkong och åtkomst." };
  }
  ...
  if (service === "Storstädning") {
    ...
    return { title: "Uppskattat pris för storstädning", beforeRut, afterRut: beforeRut * rutFactor, hours, note: "Storstädning beräknas från cirka 295 kr/tim efter RUT. Tiden påverkas mer av bostadens skick än vid återkommande hemstädning." };
  }
  ...
  if (service === "Kontorsstädning") {
    ...
    return { title: "Prisindikation för kontorsstädning", beforeRut: monthly, afterRut: monthly, monthly: true, note: "Kontorsstädning visas som konkurrenskraftig månadsindikation från cirka 29–49 kr/kvm/mån. RUT gäller inte företagsstädning." };
  }
  ...
  return { title: "Uppskattat pris för fönsterputs", beforeRut, afterRut: beforeRut * rutFactor, note: "Fönsterputs visas från 695 kr efter RUT för mindre bostäder. Exakt pris beror på antal fönster, åtkomst, våningsplan och skick." };
}
...
export default function PriceCalculator() {
  ...
  return (
    <section className="rounded-[2rem] border border-burgundy/10 bg-porcelain p-6 shadow-luxe md:p-8" id="pris-kalkylator">
      <div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.28em] text-burgundy">Kalkylator</p><h2 className="display mt-2 text-4xl font-bold text-burgundy">Beräkna pris</h2></div><div className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-burgundy text-porcelain"><Calculator /></div></div>
      <div className="grid gap-4">
        <label className="block"><span className="mb-2 block text-sm font-bold">Tjänst</span><select value={service} onChange={(event) => setSelectedService(event.target.value as Service)} className="w-full rounded-2xl border border-burgundy/10 bg-cream px-4 py-4">{services.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        <div><p className="mb-2 text-sm font-bold">Kundtyp</p><div className="grid grid-cols-2 gap-2">{customerTypes.map((item) => <button type="button" key={item} onClick={() => setSelectedCustomerType(item)} className={`rounded-2xl border px-4 py-3 text-sm font-bold transition ${customerType === item ? "border-burgundy bg-burgundy text-porcelain" : "border-burgundy/10 bg-cream text-ink"}`}>{item}</button>)}</div></div>
        <div className="grid gap-4 sm:grid-cols-2"><NumberField label="Storlek kvm" value={sqm} onChange={setSqm} /><NumberField label="Antal badrum" value={bathrooms} onChange={setBathrooms} /></div>
        <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-sm font-bold">Frekvens</span><select value={frequency} onChange={(event) => setFrequency(event.target.value as Frequency)} className="w-full rounded-2xl border border-burgundy/10 bg-cream px-4 py-4">{frequencies.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="block"><span className="mb-2 block text-sm font-bold">Postnummer</span><input value={postalCode} onChange={(event) => setPostalCode(event.target.value.slice(0, 12))} className="w-full rounded-2xl border border-burgundy/10 bg-cream px-4 py-4" placeholder="151 46" /></label></div>
      ...
      <div className="mt-7 rounded-[2rem] bg-burgundy p-6 text-porcelain"><p className="text-xs font-black uppercase tracking-[.28em] text-gold">{result.title}</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><div><p className="text-sm text-porcelain/75">Före RUT / totalpris</p><p className="display mt-1 text-4xl font-bold">{formatSek(result.beforeRut)}</p></div><div><p className="text-sm text-porcelain/75">{result.monthly ? "Prisindikation" : useRut ? "Efter RUT / kundpris" : "Kundpris utan RUT"}</p><p className="display mt-1 text-4xl font-bold text-gold">{formatSek(result.afterRut)}{result.monthly ? "/mån" : ""}</p></div></div>{result.hours && <p className="mt-4 inline-flex rounded-full bg-porcelain/10 px-4 py-2 text-sm font-bold text-gold">Uppskattad tid: cirka {result.hours.toFixed(1).replace(".", ",")} timmar</p>}<p className="mt-5 text-sm leading-7 text-porcelain/75">{result.note}</p><p className="mt-3 text-xs leading-6 text-porcelain/60">Kundtyp: {customerType}. RUT: {useRut ? "Ja" : "Nej"}. Postnummer: {postalCode || "ej angivet"}. Priset är ett uppskattat pris. Slutligt pris bekräftas efter bokningsförfrågan.</p><div className="mt-6 flex flex-col gap-3 sm:flex-row"><Link href="/#booking" className="inline-flex items-center justify-center rounded-full bg-gold px-5 py-3 text-sm font-black uppercase tracking-[.12em] text-ink">Boka städning <ArrowRight className="ml-2 h-4 w-4" /></Link><Link href="/#booking" className="inline-flex items-center justify-center rounded-full border border-gold/40 px-5 py-3 text-sm font-black uppercase tracking-[.12em] text-gold">Skicka förfrågan</Link></div></div>
      <div className="mt-5 grid gap-3 text-sm text-ink/70 md:grid-cols-3">{["RUT-avdrag visas bara för privatpersoner och tjänster där RUT normalt kan användas.", "Slutpris bekräftas innan bokningen blir bindande.", "Beräkningen används som prisindikation, inte fast offert."].map((item) => <p key={item} className="flex gap-2 rounded-2xl bg-cream p-4"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-burgundy" /> {item}</p>)}</div>
    </section>
  );
}
