        ...
        <section className={variant === "embedded" ? "grid w-full min-w-0 gap-5" : "grid gap-5 xl:grid-cols-[1fr_.75fr]"}>
          <form onSubmit={submit} onFocusCapture={trackBookingStart} className="w-full min-w-0 rounded-[2rem] border border-white/10 bg-[#252420] p-5 shadow-luxe md:p-7">
            ...
            <div className="grid gap-5">
              ...
              <div className={`grid gap-4 ${visibility.rutEligible ? "sm:grid-cols-2" : "sm:grid-cols-1"}`}><Select label={t.customerType} value={draft.customerType} options={o.customerTypes} onChange={(value) => setField("customerType", value)}/>{visibility.rutEligible && <Select label={t.rut} value={draft.rutRequested ? t.yes : t.no} options={[t.yes, t.no]} onChange={(value) => setField("rutRequested", value === t.yes)} />}</div>
