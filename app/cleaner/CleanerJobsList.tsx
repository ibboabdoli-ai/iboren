...
type OfferStatus = "accepted" | "declined";
...
export default function CleanerJobsList({ token }: { token: string }) {
  ...
  async function respond(assignmentId: string, status: OfferStatus) {
    ...
    try {
      ...
      setMessage(status === "accepted" ? "You are marked as available. Wait for admin confirmation before going to the job." : "You are marked as not available for this offer.");
    } catch (error) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[.14em]">
            <span className="rounded-full bg-cream px-3 py-1 text-ink/55 ring-1 ring-burgundy/10">{count} offers</span>
            ...
            return (
              <article key={job.assignment.id} className="rounded-[2rem] bg-cream p-5 text-sm ring-1 ring-burgundy/10 md:p-6">
                <div className="grid gap-4 lg:grid-cols-[1fr_260px] lg:items-start">
                  <div>
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-black uppercase tracking-[.14em] ${pillClass(job.assignment.status)}`}>{label(job.assignment.status)}</span>
                ...
                <div className="mt-5 grid gap-3 rounded-[1.5rem] bg-porcelain p-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <p><strong>Size:</strong> {booking.size_sqm ? `${booking.size_sqm} sqm` : "—"}</p>
