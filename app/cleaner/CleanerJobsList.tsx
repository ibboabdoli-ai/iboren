...
function shortNote(notes: string | null) {
...
function googleCalendarUrl(booking: Job["booking"]) {
  ...
  const params = new URLSearchParams({
    ...
    details: [`Service: ${booking.service}`, `Customer: ${booking.customer_name}`, `Phone: ${booking.customer_phone || "-"}`, `Size: ${booking.size_sqm ? `${booking.size_sqm} sqm` : "-"}`, `Frequency: ${booking.frequency || "-"}`, `Time: ${booking.time_window || "Flexible"}`, "Cleaner panel: https://iboren.se/cleaner"].join("\n")
  });
...
export default function CleanerJobsList({ token }: { token: string }) {
  ...
  async function respond(assignmentId: string, status: OfferStatus) {
    ...
    try {
      ...
      if (!response.ok || !result?.ok || !result.assignment) throw new Error(result?.message || "Could not update offer.");
      setJobs((current) => current.map((job) => job.assignment.id === assignmentId ? { ...job, assignment: { ...job.assignment, ...result.assignment } } : job));
      if (status === "accepted") setMessage("You are marked as available. Wait for admin confirmation before going to the job.");
      else if (status === "declined") setMessage("You are marked as not available for this offer.");
  ...
  return <section className="rounded-[2rem] bg-porcelain p-6 shadow-soft md:p-7">
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div>
        ...
        <h2 className="display text-4xl font-bold text-burgundy">My offers</h2>
        <p className="mt-3 leading-7 text-ink/65">Job offers from admin appear here. Mark yourself available or not available. The job is only confirmed after admin selects you.</p>
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[.14em]"><span className="rounded-full bg-cream px-3 py-1 text-ink/55 ring-1 ring-burgundy/10">{count} offers</span>{role && <span className="rounded-full bg-gold px-3 py-1 text-ink">{role}</span>}</div>
      </div>
    ...
    {loading ? <div className="grid min-h-32 place-items-center text-burgundy"><Loader2 className="h-7 w-7 animate-spin" /></div> : jobs.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-burgundy/20 bg-cream p-5 text-sm leading-7 text-ink/65">No job offers yet.</div> : <div className="mt-6 grid gap-5">{jobs.map((job) => {
      ...
      return <article key={job.assignment.id} className="rounded-[2rem] bg-cream p-5 text-sm ring-1 ring-burgundy/10 md:p-6">
        <div className="grid gap-4 lg:grid-cols-[1fr_260px] lg:items-start">
          <div><span className={`inline-flex rounded-full px-3 py-1 text-xs font-black uppercase tracking-[.14em] ${pillClass(job.assignment.status)}`}>{label(job.assignment.status)}</span><h3 className="display mt-3 text-3xl font-bold text-burgundy md:text-4xl">{booking.service}</h3><p className="mt-3 flex items-start gap-2 text-base font-bold text-ink"><MapPin className="mt-1 h-4 w-4 shrink-0 text-burgundy" /> {booking.address || booking.area}</p><p className="mt-2 text-sm text-ink/55">{booking.area}</p></div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {isOffer && <button type="button" disabled={isUpdating} onClick={() => respond(job.assignment.id, "accepted")} className="inline-flex items-center gap-2 rounded-full bg-green-100 px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-green-800 ring-1 ring-green-200 disabled:opacity-50">{isUpdating ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Available</button>}
          {isOffer && <button type="button" disabled={isUpdating} onClick={() => respond(job.assignment.id, "declined")} className="inline-flex items-center gap-2 rounded-full bg-red-100 px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-red-800 ring-1 ring-red-200 disabled:opacity-50">{isUpdating ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4" />}Not available</button>}
          {job.assignment.status === "accepted" && <span className="rounded-full bg-green-100 px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-green-800 ring-1 ring-green-200">Available · waiting for admin</span>}
          ...
          {showCalendar && googleUrl && <a href={googleUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-burgundy px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-porcelain"><ExternalLink className="h-4 w-4" />Google Calendar</a>}
          {showCalendar && <button type="button" disabled={calendarLoadingId === job.assignment.id} onClick={() => downloadCalendar(job.assignment.id)} className="inline-flex items-center gap-2 rounded-full bg-porcelain px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-burgundy ring-1 ring-burgundy/10 disabled:opacity-50">{calendarLoadingId === job.assignment.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}Download .ics</button>}
          {job.assignment.status === "declined" && <span className="rounded-full bg-red-100 px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-red-800 ring-1 ring-red-200">Not available</span>}
          {job.assignment.status === "completed" && <span className="rounded-full bg-ink px-5 py-3 text-xs font-black uppercase tracking-[.12em] text-porcelain">Completed</span>}
        </div>

        <div className="mt-5 grid gap-3 rounded-[1.5rem] bg-porcelain p-4 sm:grid-cols-2 lg:grid-cols-4"><p><strong>Customer:</strong> {booking.customer_name}</p><p><strong>Phone:</strong> {booking.customer_phone || "—"}</p><p><strong>Size:</strong> {booking.size_sqm ? `${booking.size_sqm} sqm` : "—"}</p><p><strong>Frequency:</strong> {booking.frequency || "—"}</p></div>
        {shortNote(booking.notes).length > 0 && <div className="mt-4 rounded-[1.5rem] bg-porcelain p-4 text-sm leading-7 text-ink/70"><strong className="text-ink">Job details</strong><ul className="mt-3 grid gap-2 md:grid-cols-2">{shortNote(booking.notes).map((line, index) => <li key={`${line}-${index}`} className="rounded-2xl bg-cream px-4 py-2">{line}</li>)}</ul></div>}
        ...
        {showTimeReport && <CleanerTimeEntryForm token={token} assignmentId={job.assignment.id} defaultDate={booking.preferred_date} />}
      </article>;
    })}</div>}
  </section>;
}
