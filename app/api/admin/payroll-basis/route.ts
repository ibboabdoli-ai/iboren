...
function nextMonthStart(start: string) {
...
export async function GET(request: Request) {
  ...
  const { data: entries, error } = await admin.supabase
    .from("time_entries")
    .select("employee_id, worked_minutes, break_minutes, travel_minutes, mileage_km")
    .eq("status", "approved")
    .gte("work_date", start)
    .lt("work_date", end)
    .returns<TimeEntry[]>();
  ...
  const { data: employees } = employeeIds.length
    ? await admin.supabase.from("employees").select("id, email, name, phone").in("id", employeeIds).returns<Employee[]>()
    : { data: [] as Employee[] };
  ...
  for (const entry of entries || []) {
    ...
    const current = grouped.get(entry.employee_id) || {
      ...
      employee_name: employee?.name || "Cleaner",
